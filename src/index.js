	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			for (const estado of estados) {
				...
				municipiosEstado = municipiosEstado.concat(
					data.map((municipio) => {
						return {
							cve_ent: municipio.cve_agee,
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			for (const municipio of municipios) {
				let url = `${URL_LOCALIDADES}${municipio.cve_ent}/${municipio.clave}`;
				let data = await fetchData(url);
				if (typeof data === 'undefined') console.log('no data url: ', url);

				if (typeof data !== 'undefined')
					localidadesMunicipio = localidadesMunicipio.concat(
						data.map((localidad) => {
							return {
								cve_ent: localidad.cve_agee,
								cve_mun: localidad.cve_agem,
